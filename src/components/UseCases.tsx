    ...
    <section className="py-24 bg-[#05020a]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
           ...
           <h3 className="font-display text-4xl text-white uppercase">Built for <span className="text-hexa-purple">Reality</span></h3>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-12">
            ...
            <div className="w-full md:w-1/3 space-y-4">
                {cases.map((c) => (
                    ...
                    >
                        <h4 className={`font-display text-lg mb-2 ${active.id === c.id ? 'text-white' : 'text-gray-400 group-hover:text-white'}`}>
                            {c.title}
                        </h4>
                        <p className="text-xs text-gray-500 font-mono-plex uppercase tracking-widest">View Case Study &rarr;</p>
                    </button>
                ))}
            </div>

            {/* Content */}
            <div className="w-full md:w-2/3 relative min-h-[400px] rounded-3xl overflow-hidden border border-gray-800 group">
                ...
                <div className="absolute bottom-0 left-0 p-8 md:p-12 z-20 w-full bg-gradient-to-t from-black via-black/80 to-transparent">
                    <h3 className="text-3xl md:text-4xl font-display text-white mb-4">{active.title}</h3>
                    <p className="text-gray-300 font-mono-plex text-sm md:text-base max-w-xl mb-8 border-l-2 border-hexa-purple pl-4">
                        {active.desc}
                    </p>
                    <div className="flex flex-wrap gap-4">
                        {active.stats.map((stat, i) => (
                            <span key={i} className="px-4 py-2 bg-white/10 backdrop-blur border border-white/20 rounded-full text-xs font-bold text-white uppercase tracking-wider">
                                {stat}
                            </span>
                        ))}
                    </div>
