...
| 4 | Platform | `Platform.tsx` | "One system, three layers" — HexaArm / HexaCore / HexaStudio. Desktop pins ~2.4 viewports and scrubs one move: the HexaStudio tile (`/media/studio-*`) grows from its place in the row into a large panel on the left while the siblings, heading and standards strip fly back and the description + Windows 64-bit download resolve on the right; phones/reduced-motion get the static stacked layout (tablet alone in frame, copy + CTA beneath) |
...
- `platform-{arm,cabinet}.webp` — the two sibling tiles in the Platform section (HexaArm wrist photo, HexaCore cabinet crop of the system photo).
- `studio-{2560,1600}.{avif,webp}` + `studio-1600.jpg` — HexaStudio tablet render (1.646, flattened on black); the card that grows in the pinned Platform section (it replaced the old HMI tile). Master `new_pictures/HexaStudio Tablet.png`.
- Brand icons in `public/` from `new_pictures/logo-master.png` — `logo-nav.png` (navbar mark), `favicon-{48x48,96x96}.png` + `favicon.ico`, `apple-touch-icon.png`, `logo-{192,512}.png` (PWA maskable, padded on black). Trimmed transparent margin; favicons stay transparent, touch/PWA icons sit on black.
- `public/og-cover.jpg` — 1200×630 social card (WhatsApp/Telegram/Google). The **hero system photo** (zoomed so the bottom black crops off) with a small logo + `HEXAKINETICA` lockup top-left, rendered from HTML with the real Michroma font. Not regenerated by `npm run media` (sharp has no browser/font pipeline) — regenerate manually if the brand or photo changes.
